...
系统提供便捷的启动/停止脚本，支持 Windows 和 Linux 平台，位于项目 bin 目录下。
...
| 配置方式 | 说明 | 优先级 |
|----------|------|--------|
| 脚本参数 | `start.bat 9090` 或 `./start.sh 9090` | 最高 |
| 配置文件 | `config.yaml` 中 `server.port` | 中 |
| 默认值 | 8080 | 最低 |
...
**Windows 进程管理**:
- 通过端口（默认8080）查找进程
- 使用 `netstat -aon` 命令识别进程 PID
- 使用 `taskkill /F /PID` 停止进程

**Linux 进程管理**:
- 使用 PID 文件（`bin/dte-diag.pid`）记录进程标识
- PID 文件在启动时自动创建，停止时自动删除
- 通过 `lsof -t -i:${PORT}` 备用查找进程
- 优雅停止：先发送 SIGTERM，2秒后检查是否需要 SIGKILL

**PID 文件说明**:

| 属性 | 说明 |
|------|------|
| 文件位置 | `bin/dte-diag.pid` |
| 创建时机 | start.sh 启动时 |
| 删除时机 | stop.sh 停止时 |
| 内容格式 | 单行数字，为进程 PID |
